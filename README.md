...
```
Craig → Hanna (fleet manager + orchestrator + money)
         → Sam (tickets + live proof)
         → Burtha (GitHub + shell)
         → Kayla (sales + GHL + texts Approve-gated + recruiter)
         → Maddi (brand + video)
         → Oma (research + LEADS-AND-INSTALLS disk)
Craig → dr eggbot (Grok Bot design / CreateAgent)
```
...
|------|----------|
| `00-INDEX/` | Pack index |
| `01-live-profiles/` | Live Grok `profile.json` snapshots |
| `02-personas-grok-fleet/` | `~/.grok/fleet/PERSONAS` |
| `03-personas-shared-desk/` | Shared desk PERSONAS |
| `04-agent-memory-grok/` | Grok AGENT-MEMORY |
| `05-agent-memory-shared/` | Shared AGENT-MEMORY |
| `06-boot/` | Worker boot + connector playbooks |
| `07-lanes/` | Fleet lanes / NOW / directives |
